...
interface ShapeRendererProps {
  ...
  color?: string;
  outline?: boolean;
  hole?: boolean; // Montessori-style recessed hole
  inactive?: boolean; // Grayed out inactive state
  className?: string;
}
...
const SHAPE_PATHS: Record<ShapeType, string> = {
  ...
  semicircle: 'M 5 50 A 45 45 0 0 1 95 50 L 5 50 Z',
  ...
  heptagon: 'M 50 5 L 85 20 L 97 55 L 79 88 L 50 95 L 21 88 L 3 55 L 15 20 Z',
  ...
  trapezoid: 'M 20 20 L 80 20 L 95 80 L 5 80 Z',
};

// Consistent dark gray for inactive shapes
...
export function ShapeRenderer({
  ...
  const baseColor = color || SHAPE_COLORS[shapeType];
  const path = SHAPE_PATHS[shapeType];

  // Inactive mode: consistent dark gray for unavailable shapes
  if (inactive) {
    return (
      <svg
        width={size}
  ...
  // Hole mode: dark with subtle color tint and high contrast outline
  if (hole) {
    const holeColor = darkenColor(baseColor, 0.8); // 80% darker - subtle color hint
    ...
    return (
      ...
      >
        ...
        {/* White outer glow/outline for visibility in dark mode */}
        <path
          d={path}
          fill="none"
          stroke="white"
